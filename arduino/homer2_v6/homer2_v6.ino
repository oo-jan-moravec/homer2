...
static int encKp = 12;   // P gain: lower = gentler
...
void loop() {
  ...
  if (readLine(line, sizeof(line))) {
    ...
    // ENC config: ENC 0 | ENC 1 | ENC 1 <kp> <max>
    if (line[0] == 'E' && line[1] == 'N' && line[2] == 'C' && (line[3] == '\0' || line[3] == ' ')) {
      int en = 0, kp = 0, maxc = 0;
      int n = sscanf(line + 3, "%d %d %d", &en, &kp, &maxc);
      if (n >= 1) {
        encCorrectionEnabled = (en != 0);
        if (n >= 3 && encCorrectionEnabled && kp > 0 && maxc > 0) {
